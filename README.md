...
- **PACKET 模式（默认）**：通过 Mixin 拦截  方法，在玩家下落时将  的  字段设为 。服务器收到后认为玩家在地面，不会计算摔落伤害。
- **SIMPLE 模式**：在客户端 tick 时直接重置 。注意：此模式在纯客户端修改，部分服务器的反作弊可能不认可。

## 安装

1. 安装 [Fabric Loader](https://fabricmc.net/) >= 0.18.4
2. 安装 [Fabric API](https://modrinth.com/mod/fabric-api) >= 0.155.2
3. 从 [Releases](https://github.com/led-un/FlightMod-Fabric/releases) 下载 ，放入  目录
